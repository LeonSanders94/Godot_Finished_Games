...
func _physics_process(delta):
	...
	# Original movement (Script 2 pattern)
	if being_pulled:
		var player = get_tree().get_first_node_in_group("Playergroup")
		if player:
			var direction = (player.global_position - global_position).normalized()
			position += direction * spd * delta  # + for positive speed
	else:
		position -= transform.y * spd * delta  # Original falling movement
...
func _on_body_entered(body):
	...
	if body.has_method("increase_money_tier2"):  # Tier 1 uses increase_money
		collected = true
		
		# Calculate final money with all bonuses
		var base_money = 13 + (Player.Wave * 2)
		...
		print("Collected ", final_money, " tier 2 money (", base_money, " base + bonuses)")
